...
let GraphQLTestDirective, GraphQLTestDirective1, schema;


describe('GraphQLCustomDirective', () => {

    before(() => {
        const resolve = function(resolve, source, { by }, schema, info) {
        ...
        GraphQLTestDirective1 = new GraphQLCustomDirective({
            name: 'duplicate1',
            description:
              'duplicate the string sperating them with space',
            locations: [
    ...
    it('expected directive to alter execution of graphql and result test test', (done) => {
        const query = `{ value(input: "test") @duplicate1 }`,
            passServer = true,
            directives = [ GraphQLTestDirective, GraphQLTestDirective1 ],
    ...
    it('expected directive to alter execution of graphql and result test test', (done) => {
        const query = `{ value(input: "test") @duplicate1 @duplicate(by:3) }`,
            directives = [ GraphQLTestDirective, GraphQLTestDirective1],
    ...
    it('expected directive to alter execution of graphql and result test test test', (done) => {
        const query = `{ value @duplicate1 @duplicate(by:3) }`,
